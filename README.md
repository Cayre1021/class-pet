...
### Web 版本
- 当前仓库前端包版本：`0.1.1`（见 `package.json`）
- 当前网页端部署基路径：`/class-pet/`
- 适用场景：GitHub Pages、浏览器本地运行、局域网演示

### Android 版本
- 当前 Android `versionName`：`0.1.1`
...
### v0.1.1 · 2026-05-11 14:16
- 已为不同宠物类型改用独立的高对比背景色，避免蓝色小鱼配蓝底、绿色龙配绿底这类混色问题。
- 破壳后的宠物不再复用蛋壳背景色，教师端、大屏页、学生查询页和互动卡片统一使用同一套新配色。
- 版本号已同步更新到 Web、Android 与桌面打包元数据，便于后续继续发布 0.1.1 产物。
